...
#####################################################################################
    ...
    #   series. This implementation is an extension of the algorithm proposed by 
    #   Baydogan et al. In this implementation every channel of every input train two  
    #   RandomForest classifier (one for the CPE used to make the codebook, and one 
    #   for the CPE output)    
    #</summary>
class BoF:
    #<summary>
    ...
    def fit(self, X, Y):   
        ...
        h_train_t = []
        for k in range(self.channels):
            h_train_t.append(self.aHistrogramas(f_train_t[k], X.shape[0], self.forests[k]))        
        h_train = np.concatenate(h_train_t, axis=1)        
        del h_train_t
        del f_train_t
        gc.collect()
        tolerance=0.05
    ...
    #<summary>
    #   predict:  Starts the prediction process of the BoFM algorithm.
    #   <var name="X">
    ...
    def predict(self, X):        
        if self.setFit:
            ...
            h_test_t = []
            for k in range(self.channels):
                h_test_t.append(self.aHistrogramas(f_test_t[k], X.shape[0], self.forests[k], fase="test"))
            h_test = np.concatenate(h_test_t,axis=1)      
